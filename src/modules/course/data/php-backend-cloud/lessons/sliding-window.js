export default {
  ...
  segments: [
    ...
    { type: 'p', html: 'A <em>window</em> is a contiguous subarray or substring defined by two indices <code>[left, right]</code>. Instead of computing the answer for every possible window from scratch (O(n²) or O(nk)), we <strong>slide</strong> the window: when the right boundary advances, we add the new element\'s contribution; when the left boundary advances, we remove the departing element\'s contribution. Each element enters and leaves the window at most once — giving O(n) total work.' },
    { type: 'p', html: 'The mental model: think of the window as a conveyor belt. Elements flow in from the right, elements fall off the left. Your job is to maintain a running statistic (sum, max, frequency count) as the belt moves.' },

    { type: 'callout', style: 'info', title: 'When Sliding Window Applies', html: 'The problem asks about a <strong>contiguous</strong> subarray or substring, and the window statistic has a <strong>monotonic</strong> relationship with window size (larger window = larger sum for non-negative numbers) or a clear shrink condition. If elements can be negative, the sum is not monotonic — Kadane\'s algorithm or prefix sums may be needed instead.' },

    { type: 'h2', text: 'Fixed-Size Window: Maximum Sum Subarray of Size k' },
    { type: 'p', html: 'The simplest variant. Compute the sum of the first k elements (initial window). Then shift: subtract the element falling off the left, add the element entering from the right. Track the running maximum. O(n) time, O(1) space.' },

    { type: 'code', lang: 'php', label: 'PHP — Fixed Window: Max Sum Subarray', code: `&lt;?php
...
print_r(findAnagrams('cbaebabacd', 'abc'));  // [0, 6]` },
    ...
    { type: 'h2', text: 'Variable-Size Window: Longest Substring Without Repeating Chars' },
    { type: 'p', html: 'Here the window size is not fixed — we grow it as long as the invariant holds (all unique characters) and shrink it when the invariant breaks (a duplicate enters). The shrink condition is precise: jump <code>left</code> to <code>lastSeen[char] + 1</code> to immediately exclude the duplicate, rather than incrementing one step at a time.' },

    { type: 'code', lang: 'php', label: 'PHP — Variable Window: Longest Unique Substring', code: `&lt;?php
...
echo longestSubstringKDistinct('eceba', 2);       // 3 ("ece")` },
    ...
    { type: 'callout', style: 'tip', title: 'Jump vs Creep for the Left Pointer', html: 'In the unique-substring problem, when a duplicate is found you can jump <code>left</code> directly to <code>lastSeen[char] + 1</code> rather than incrementing one step at a time. This is still O(n) total but avoids redundant iterations. In the "at most k distinct" variant you must creep (increment one step) because the shrink condition involves a count that decreases gradually.' },

    { type: 'h2', text: 'Variable-Size Window: Minimum Window Substring' },
    { type: 'p', html: 'One of the hardest sliding window problems: find the minimum length substring of <code>s</code> that contains all characters of <code>t</code>. Strategy: expand right until the window is valid (contains all of t\'s chars); then shrink left as far as possible while keeping it valid; record the minimum. Use a "have vs need" counter to check validity in O(1).' },

    { type: 'code', lang: 'php', label: 'PHP — Minimum Window Substring', code: `&lt;?php
...
echo minWindow('a', 'aa');             // ""` },
    ...
    { type: 'h2', text: 'Sliding Window Maximum — Monotonic Deque' },
    { type: 'p', html: 'Find the maximum in every window of size k. Naive: O(nk). Optimal: O(n) using a <em>monotonic decreasing deque</em> that stores indices. The deque\'s front always holds the index of the current window\'s maximum. Elements are evicted from the back when they are smaller than the incoming element (they can never be a future maximum), and from the front when they fall outside the window.' },

    { type: 'code', lang: 'php', label: 'PHP — Sliding Window Maximum (Monotonic Deque)', code: `&lt;?php
// O(n) time, O(k) space
...
// [3, 3, 5, 5, 6, 7]` },
    ...
    { type: 'callout', style: 'warn', title: 'When Sliding Window Breaks Down', html: 'Sliding window requires that the window statistic behaves monotonically relative to window size. With negative numbers, a larger sum subarray doesn\'t guarantee a larger window — the sum can shrink. For "maximum subarray sum" with negative numbers, use Kadane\'s algorithm. For "subarray sum equals k" with negatives, use prefix sums + a hash map.' },

    { type: 'h2', text: 'Complexity Analysis — The O(n) Argument' },
    { type: 'p', html: 'The nested loop in variable-size sliding window looks O(n²) but is actually O(n). The key insight: the inner <code>while</code> (shrink) loop moves <code>left</code> forward. <code>left</code> starts at 0 and can advance at most n times across the entire outer loop. So the total number of inner iterations across all outer iterations is at most n. Total work: O(n) outer + O(n) inner = O(2n) = O(n).' },

    { type: 'table', headers: ['Problem', 'Window Type', 'Data Structure', 'Time', 'Space'], rows: [
      ['Max sum subarray of size k', 'Fixed', 'Running sum', 'O(n)', 'O(1)'],
      ['Find all anagram positions', 'Fixed', 'Freq array (26 ints)', 'O(n)', 'O(1)'],
      ['Longest unique substring', 'Variable', 'Hash map (last index)', 'O(n)', 'O(k)'],
      ['Longest with at most k distinct', 'Variable', 'Hash map (count)', 'O(n)', 'O(k)'],
      ['Minimum window substring', 'Variable', 'Two freq maps + counter', 'O(n+m)', 'O(m)'],
      ['Sliding window maximum', 'Fixed', 'Monotonic deque', 'O(n)', 'O(k)'],
    ]},

    { type: 'qa', pairs: [
      {
        q: 'Q: How do you decide between a fixed-size and variable-size sliding window?',
        a: 'Look at what the problem gives you and what it asks for. <strong>Fixed-size</strong>: the window length k is given explicitly and never changes (e.g., "max sum of k elements", "find anagram positions of p in s" — window size = len(p)). <strong>Variable-size</strong>: you are asked to find the longest or shortest window satisfying some constraint, and the window must grow and shrink based on whether the constraint is met (e.g., "longest substring without repeating chars", "minimum window containing all chars"). In variable-size problems, you always have a shrink condition — write it out before coding.',
      },
      {
        q: 'Q: Walk me through the "have vs need" pattern used in Minimum Window Substring. Why is it O(1) to check window validity?',
        a: 'We precompute <code>required</code> = number of distinct characters in t that must be satisfied. We maintain <code>have</code> = number of those characters whose count in the current window meets or exceeds the requirement. When <code>have === required</code>, the window is valid. Incrementing/decrementing <code>have</code> happens at most once per character per pointer move: only when a character\'s window count crosses its requirement threshold (from below to exactly equal, or from equal to below). This means each pointer advances O(n) times total and the validity check itself is a single integer comparison — O(1). Without this trick, naively comparing two full frequency maps would be O(m) per step.',
      },
      {
        q: 'Q: Explain why the sliding window nested loop is O(n) and not O(n²).',
        a: 'Use the amortised argument. The outer loop runs exactly n times (right = 0..n-1). The inner (shrink) loop advances the <code>left</code> pointer. <code>left</code> is a monotonically increasing index that starts at 0 and can reach at most n. Therefore, across the entire execution, the inner loop body can execute at most n times in total — regardless of how many outer iterations there are. Total operations: n (outer) + n (inner total) = 2n = O(n). This is the same argument as for the amortised cost of a stack-based approach.',
      },
    ]},

    { type: 'keypoints', title: 'Key Points to Remember', items: [
      'Fixed-size window: window length k is given; shift by subtracting left element and adding right element each step',
      'Variable-size window: grow right unconditionally; shrink left when the invariant breaks; track the best answer after each valid state',
      'The nested loop is O(n) not O(n²): left pointer moves at most n times total across the entire execution',
      'Sliding window requires monotonicity: works for non-negative sums, character counts; breaks for negative-number sum problems',
      '"Have vs need" counter avoids O(m) frequency map comparison — reduces validity check to a single integer comparison',
      'Sliding window maximum needs a monotonic deque — a simple running max won\'t work because the maximum can leave the window',
      'For all anagram problems: fixed window of size len(p) + two 26-element frequency arrays — O(n) time, O(1) space',
    ]},
  ],
};
