export default {
  ...
  segments: [
    { type: 'h2', key: 'h_execution_order' },
    { type: 'p', key: 'p_execution_order_intro' },
    { type: 'ul', key: 'ul_execution_order' },
    {
      type: 'code',
      lang: 'sql',
      label: 'SQL — Core Queries',
      code: `-- Full example: top 5 customers by 2024 revenue
SELECT
    c.id,
    c.name,
...
FROM customers;`,
    ...
    { type: 'h2', key: 'h_joins' },
    {
      ...
      lang: 'sql',
      label: 'SQL — JOINs',
      code: `-- INNER JOIN: only rows matching in BOTH tables
...
LEFT JOIN employees m ON e.manager_id = m.id;`,
    ...
    { type: 'h2', key: 'h_ctes_windows' },
    {
      type: 'code',
      lang: 'sql',
      label: 'SQL — CTEs & Windows',
      code: `-- CTE: named temporary result set — makes complex queries readable
...
SELECT REPEAT('  ', depth) || name AS hierarchy FROM tree;`,
  ...
  bodyTexts: {
    h_execution_order: 'SQL Execution Order',
    p_execution_order_intro: 'SQL is written in one order but executed in a different order. Understanding this is critical — it explains why you cannot use SELECT aliases in WHERE, and why HAVING exists:',
    ul_execution_order: [
      '<strong>FROM & JOIN</strong> — which tables?',
      '<strong>WHERE</strong> — filter rows (before aggregation, uses indexes)',
      '<strong>GROUP BY</strong> — group rows into buckets',
      '<strong>HAVING</strong> — filter groups (after aggregation)',
      '<strong>SELECT</strong> — which columns to return',
      '<strong>DISTINCT</strong> — remove duplicates',
      '<strong>ORDER BY</strong> — sort results',
      '<strong>LIMIT / OFFSET</strong> — pagination',
    ],
    h_joins: 'All JOIN Types',
    h_ctes_windows: 'CTEs & Window Functions',
    keypoints: {
      title: 'Key Points to Remember',
      items: [
        'Execution order: FROM → WHERE → GROUP BY → HAVING → SELECT → ORDER BY → LIMIT',
        'WHERE filters rows before grouping (uses indexes). HAVING filters groups after aggregation.',
        'COUNT(*) counts all rows including NULLs. COUNT(column) ignores NULLs — common trap!',
        'INNER JOIN = only matches. LEFT JOIN = all left rows, NULL for unmatched right.',
        'Anti-join pattern: LEFT JOIN + WHERE right.id IS NULL — find rows with no match',
        'CTEs (WITH clause): name intermediate results for readable complex queries',
        'Window functions: RANK, ROW_NUMBER, LAG, LEAD, SUM OVER — aggregate without collapsing',
      ],
    },
  },
};
