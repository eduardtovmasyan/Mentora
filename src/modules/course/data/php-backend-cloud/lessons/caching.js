...
function xfetch(string $key, int $ttl, float $beta, callable $compute): mixed
{
    ...
    { type: 'callout', style: 'tip', title: 'Tip', html: 'Laravel\'s <code>Cache::lock()</code> is backed by Redis atomic SETNX under the hood. Always use a try/finally block to ensure the lock is released even if the callback throws an exception, preventing permanent deadlocks.' },

    { type: 'h2', text: 'HTTP Caching: Cache-Control, ETags, and CDN' },
    { type: 'p', html: 'HTTP caching is free performance for public content. The <code>Cache-Control</code> header tells browsers and CDNs how long to cache a response. <code>ETag</code> is a fingerprint of the response; on the next request the browser sends <code>If-None-Match</code> and the server can respond with 304 Not Modified (no body transfer). <code>Last-Modified</code> / <code>If-Modified-Since</code> is a time-based equivalent. CDNs (CloudFront, Cloudflare) respect these headers and cache responses at edge locations globally, eliminating round-trips to your origin server for cacheable content.' },
    { type: 'code', lang: 'php', label: 'PHP', code: `&lt;?php
...
// no-cache         — must revalidate before serving (ETag flow)` },
    ...
    { type: 'h2', text: 'Redis in Laravel' },
    { type: 'p', html: 'Laravel\'s cache abstraction supports multiple drivers. Redis is the preferred driver for production because it supports cache tags, atomic operations (INCR, SETNX), pub/sub, and persistence. Configure your <code>.env</code> to use the Redis driver and ensure the Predis or PhpRedis extension is installed. Use separate Redis databases or key prefixes to isolate cache, session, and queue data. Enable Redis Cluster or Redis Sentinel for high availability in production.' },
    { type: 'code', lang: 'php', label: 'PHP', code: `&lt;?php
...
Redis::pipeline(function ($pipe) use ($userId) {
    ...
    { type: 'qa', pairs: [
      {
        q: 'Q: What is cache penetration and how do you prevent it?',
        a: 'Cache penetration occurs when requests are made for keys that do not exist in either the cache or the database — every request misses the cache and hits the database. This can be exploited as a denial-of-service attack. Prevention strategies: (1) <strong>Negative caching</strong> — cache the "not found" result with a short TTL (e.g., 30 seconds) so repeated requests for the same non-existent key are served from cache. (2) <strong>Bloom filter</strong> — maintain a bloom filter of all valid IDs; if the requested ID is not in the bloom filter, return 404 immediately without touching the cache or database. Laravel does not have a built-in bloom filter, but you can use a Redis bit field as an approximation.',
      },
      {
        q: 'Q: What is the difference between cache eviction and cache invalidation?',
        a: '<strong>Cache eviction</strong> is what happens when the cache is full and needs to make room for new entries — the cache itself decides which entries to remove based on an eviction policy (LRU — least recently used, LFU — least frequently used, TTL expiry, random). Eviction is a resource management mechanism. <strong>Cache invalidation</strong> is an explicit, business-logic-driven action: you tell the cache to remove or update a specific entry because the underlying data has changed. Invalidation is about correctness; eviction is about memory management. Both can cause cache misses, but only poor invalidation strategy causes stale data to be served.',
      },
      {
        q: 'Q: When should you NOT use cache tags in Laravel?',
        a: 'Cache tags are only supported by the Redis and Memcached drivers. If you are using the file or database cache driver (common in small apps or testing), cache tags will throw an exception. Additionally, cache tags in Redis use a secondary data structure to track which keys belong to a tag — this adds memory overhead and one extra Redis round-trip per cache operation. For extremely high-throughput scenarios where every microsecond counts, directly managing cache keys without tags and accepting more explicit invalidation logic can be faster. Always benchmark before optimizing at this level.',
      },
    ]},

    { type: 'keypoints', title: 'Key Points to Remember', items: [
      'Cache-aside (lazy loading) is the most flexible pattern; the application controls all cache reads and writes explicitly.',
      'Write-through keeps cache and DB in sync at the cost of write latency; write-behind sacrifices durability for write speed.',
      'TTL alone is not a cache invalidation strategy — combine event-driven invalidation with TTL as a safety net.',
      'Cache stampede happens when a hot key expires and all concurrent requests hit the DB simultaneously; solve it with mutex locks, probabilistic early expiration, or background pre-warming.',
      'Use cache tags in Laravel (Redis driver) to group related keys so an entire logical group can be invalidated atomically.',
      'HTTP Cache-Control and ETag headers enable browser and CDN caching — always set <code>private, no-store</code> for authenticated or sensitive responses.',
      'Add random jitter (±10–20%) to TTLs on hot keys to prevent synchronized mass expiry across a Redis cluster.',
      'Negative caching (caching "not found" results) prevents cache penetration attacks that would otherwise hammer the database.',
    ]},
  ],
};
